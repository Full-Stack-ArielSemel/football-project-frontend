...
export default function GeneralTable(){
    ...
    async function fetchData() {
        await axios.get("http://localhost:8989/get-general-table-by-league-id",{
            params:{
        ...
        <div>
            {
                JSON.parse(window.localStorage.getItem("save")) === 0 ?
                    <h1 style={{color: "red", alignItems: "center",marginLeft:"380px"}}>FOR SHOWING TABLE INFO SELECTING LEAGUE IS
                        REQUIRED!</h1>
                    :
                    <table>
                        <thead>
                        ...
                        {sortedTable.map((item,index) => (
                            <tr>
                                ...
                                <td style={{textAlign:"left"}}>
                                    ...
                                    <span style={{marginLeft:"7px"}}>{item.name}</span></td>
